...
class Sprite {
    constructor(public texture: string = 'default', public color: string = '#ffffff') {}
}

class Collider {
    constructor(public solid: boolean = true, public trigger: boolean = false) {}
}
...
class Enemy {
    constructor(public type: string = 'basic', public speed: number = 50) {}
}
...
// Initialize engine
const game = new Engine(60, true); // 60 FPS, debug mode
...
game.createSystem('CollisionSystem', {
    ...
    act: (entity, position, size, collider) => {
        ...
        for (const other of allEntities) {
            ...
            if (!other.hasComponent(Position) || !other.hasComponent(Size) || !other.hasComponent(Collider)) continue;
            ...
            if (position.x < otherPos.x + otherSize.width &&
                ...
                if (collider.solid && otherCollider.solid) {
                    ...
                    const dy = (position.y + size.height / 2) - (otherPos.y + otherSize.height / 2);
...
// Enemy AI System
game.createSystem('EnemyAISystem', {
    all: [Enemy, Position, Velocity],
    withoutTags: ['dead']
}, {
    priority: 80,
    act: (entity, enemy, position, velocity) => {
        // Find player

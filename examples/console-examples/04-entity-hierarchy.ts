...
game.createSystem('TransformPropagationSystem', {
    ...
    act: (entity, transform, worldTransform) => {
        if (!entity.parent) {
            ...
            if (parent.hasComponent(WorldTransform)) {
                ...
                worldTransform.x = parentWorld.x + 
                    (transform.localX * cos - transform.localY * sin) * parentWorld.scale;
                worldTransform.y = parentWorld.y + 
...
game.createSystem('OrbitSystem', {
    all: [Transform, Orbit]
}, {
    priority: 85,
    act: (entity, transform, orbit) => {
...
game.createSystem('JointSystem', {
    all: [Transform, Joint]
}, {
    priority: 80,
    act: (entity, transform, joint) => {
        if (joint.type === 'rotating') {
            // Constrain rotation to limits
            transform.localRotation = Math.max(joint.minAngle, 
...
game.createSystem('VisualizationSystem', {
    all: [WorldTransform, Visual]
}, {
    priority: 10,
    act: (entity, worldTransform, visual) => {
...
function printHierarchy(entity: any, depth: number) {
    const indent = '  '.repeat(depth);
    const visual = entity.hasComponent(Visual) ? entity.getComponent(Visual) : null;
    const worldTransform = entity.hasComponent(WorldTransform) ? entity.getComponent(WorldTransform) : null;
...
solarSystem.addComponent(Transform, 400, 300, 0, 1)
...
mercury.addComponent(Transform, 60, 0, 0, 0.5)
       .addComponent(WorldTransform)
...
earth.addComponent(Transform, 150, 0, 0, 1)
     .addComponent(WorldTransform)
...
phobos.addComponent(Transform, 20, 0, 0, 0.2)
      .addComponent(WorldTransform)
...
deimos.addComponent(Transform, 30, 0, 0, 0.15)
      .addComponent(WorldTransform)
...
robotBase.addComponent(Transform, 200, 450, 0, 1)
         .addComponent(WorldTransform)
         .addComponent(Visual, 'square', 40, '#333333', 'Base')
         .addTag('robot');

const robotArm1 = game.createEntity('RobotArm1');
robotArm1.addComponent(Transform, 0, -30, 0, 1)
         .addComponent(WorldTransform)
...
robotArm2.addComponent(Transform, 0, -60, 0, 0.8)
         .addComponent(WorldTransform)
...
robotGripper.addComponent(Transform, 0, -40, 0, 0.6)
            .addComponent(WorldTransform)
...
character.addComponent(Transform, 600, 200, 0, 1)
         .addComponent(WorldTransform)
...
let detachTimer = 0;
game.createSystem('DetachAttachSystem', {
    tags: ['character']
}, {
    priority: 50,
    act: (entity) => {
...
game.createSystem('HierarchyStatsSystem', {
    all: []
}, {
    priority: 5,
    before: () => {
        ...
        let maxDepth = 0;
        function calculateDepth(entity: any, depth: number = 0) {
...
setTimeout(() => {
    ...
    roots.forEach(root => printHierarchy(root, 0));
}, 45000);
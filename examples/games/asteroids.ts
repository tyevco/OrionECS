 ...
 */
class Rotation {
  constructor(public angle: number = 0, public speed: number = 0) {}
}
 ...
 */
class Health {
  constructor(public value: number = 1, public max: number = 1) {}
}
...
engine.createSystem('PlayerControlSystem',
  ...
  {
    priority: 900,
    act: (entity: EntityDef, player: Player, input: InputState, rotation: Rotation, velocity: Velocity, position: Position) => {
      ...
      if (input.shoot && player.shootCooldown <= 0) {
        ...
        // Publish shoot event
        engine.messageBus.publish('player-shoot', { position: bulletPos }, 'PlayerControlSystem');
      }
    }
  },
  true // Fixed update
...
function damagePlayer(entity: EntityDef): void {
  ...
  if (health.value <= 0) {
    if (player.lives > 0) {
      ...
      engine.messageBus.publish('player-respawn', { livesLeft: player.lives }, 'CollisionSystem');
    } else {
      // Game over
      destroyEntity(entity);
 ...
 */
engine.createSystem('BulletLifetimeSystem',
  { all: [Bullet] },
  {
    priority: 300,
 ...
 */
engine.createSystem('AsteroidSpawnerSystem',

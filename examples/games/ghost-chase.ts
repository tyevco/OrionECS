...
function registerPredicates(engine: ReturnType<typeof createEngine>) {
    ...
    dt.predicate('ghost.isEaten', (entity) => entity.hasComponent(Eaten));

    dt.predicate('ghost.isInHouse', (entity) => entity.hasComponent(InGhostHouse));

    dt.predicate('ghost.isFrightened', (entity) => entity.hasComponent(Frightened));

    // --- Personality Predicates ---

    dt.predicate(
        'ghost.isBlinky',
        (entity) => entity.getComponent(Ghost)?.personality === 'blinky'
    );

    dt.predicate('ghost.isPinky', (entity) => entity.getComponent(Ghost)?.personality === 'pinky');

    dt.predicate('ghost.isInky', (entity) => entity.getComponent(Ghost)?.personality === 'inky');

    dt.predicate('ghost.isClyde', (entity) => entity.getComponent(Ghost)?.personality === 'clyde');

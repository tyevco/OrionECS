...
import { type Engine, EngineBuilder, type EntityDef } from '@orion-ecs/core';
...
export class InputHistory {
    ...
    getInputsSince(
        sequence: number
    ): Array<{ sequence: number; input: ClientInput; timestamp: number }> {
...
export class MultiplayerServer {
                ...
                act: (entity: EntityDef, pos: Position, vel: Velocity, input: ClientInput) => {
                    ...
                    if (input.shoot && entity.hasComponent(Weapon)) {
                        ...
                        if (weapon.canFire(currentTime)) {
                            ...
                            const _projectile = this.createProjectile(
                                pos.x,
                                pos.y,
                    ...
                    for (const projectile of projectiles) {
                        ...
                        for (const player of players) {
                            ...
                            if (distance < projCol.radius + playerCol.radius) {
                                ...
                                console.log(
                                    `[Server] Player hit! Health: ${health.current}/${health.max}`
                                );
                                ...
                                if (health.isDead()) {
                                    ...
                                    const shooterPlayer = players.find(
                                        (p) =>
                                            p.getComponent(NetworkedEntity).ownerId ===
                                            projNet.ownerId
                                    );
    ...
    private handleInput(playerId: symbol, message: ClientMessage & { type: 'input' }): void {
        ...
        const _position = player.getComponent(Position);
    ...
    private handlePing(_clientId: string, _timestamp: number): void {
        // Send pong
        // sendToClient(clientId, {
        //   type: 'pong',
        //   timestamp,
...
export class MultiplayerClient {
    ...
    private setupSystems(): void {
        ...
        this.engine.createSystem(
            ...
            {
                priority: 1000,
                act: (_entity: EntityDef, pos: Position, vel: Velocity, input: ClientInput) => {
                    const speed = MULTIPLAYER_CONFIG.playerSpeed;
        ...
        this.engine.createSystem(
            ...
            {
                priority: 1000,
                act: (_entity: EntityDef, pos: Position, interpolation: InterpolationState) => {
    ...
    private createNetworkedEntity(entityData: any): EntityDef {
        ...
        if (entityData.type === 'player') {
            ...
            entity.addComponent(
                Health,
                entityData.health?.current || 100,
                entityData.health?.max || 100
            );
            ...
            entity.addComponent(
                Velocity,
                entityData.velocity?.dx || 0,
                entityData.velocity?.dy || 0
            );

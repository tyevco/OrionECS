...
engine.createSystem(
    ...
    {
        priority: 1000,
        act: (_entity: EntityDef, input: Input) => {
...
engine.createSystem(
    ...
    {
        priority: 900,
        act: (_entity: EntityDef, velocity: Velocity, rigidBody: RigidBody) => {
            const dt = 1 / 60;
    ...
    { all: [Position, BoxCollider, Collectible] },
    {
        ...
        act: (
            ...
            for (const player of players) {
                ...
                if (
                    dx < (collider.width + playerCol.width) / 2 &&
...
engine.createSystem(
    ...
    {
        priority: 500,
        act: (_entity: EntityDef, anim: AnimationController, state: CharacterState) => {
            const dt = 1 / 60;
...
engine.createSystem(
    ...
    {
        priority: 400,
        act: (_entity: EntityDef, camera: Camera) => {
            const players = engine.getEntitiesByTag('player');
...
engine.createSystem(
    ...
    {
        priority: 450,
        act: (_entity: EntityDef, velocity: Velocity, sprite: Sprite) => {
            if (Math.abs(velocity.dx) > 10) {
...
function loadLevel(levelData: LevelData): void {
    ...
    console.log(
        `Level loaded: ${levelData.platforms.length} platforms, ${levelData.enemies.length} enemies, ${levelData.coins.length} coins`
    );
}
...
engine.messageBus.subscribe('player-jump', (_message) => {
    // Play jump sound

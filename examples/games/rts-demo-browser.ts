...
canvas.addEventListener('contextmenu', (e) => {
    ...
    issueCommand({
        ...
        target: {
            x: mouse.worldX,
            y: mouse.worldY,
            clone: () => ({ x: mouse.worldX, y: mouse.worldY }),
        } as any,
    });
});
...
function render() {
    ...
    for (const entity of buildings) {
        ...
        if (entity.hasComponent(Health)) {
            ...
            ctx.fillStyle =
                health.percentage > 60 ? '#4aff4a' : health.percentage > 30 ? '#ffff4a' : '#ff4a4a';
            ctx.fillRect(
                pos.x - barWidth / 2,
                barY,
                (barWidth * health.percentage) / 100,
                barHeight
            );
        }
    }
...
function updateUI() {
    ...
    const selectedEntities = gameState?.selectedEntities ?? new Set<symbol>();
...
function updateCamera() {
    ...
    camera.x = Math.max(
        0,
        Math.min(RTS_CONFIG.worldSize.width - canvas.width / camera.zoom, camera.x)
    );
    camera.y = Math.max(
        0,
        Math.min(RTS_CONFIG.worldSize.height - canvas.height / camera.zoom, camera.y)
    );
}

...
function sendToClient(client: any, message: any): void {
...
function createPlayer(clientId: string, playerName: string): EntityDef {
  ...
  console.log(`[Server] Created player: ${playerName} at (${spawnX.toFixed(1)}, ${spawnY.toFixed(1)})`);
...
serverEngine.createSystem(
  ...
  { all: [Position, Velocity] },
  {
    ...
    act: (entity: EntityDef, position: Position, velocity: Velocity) => {
      ...
      if (position.x > SERVER_CONFIG.worldSize.width) position.x -= SERVER_CONFIG.worldSize.width;
      if (position.y < 0) position.y += SERVER_CONFIG.worldSize.height;
      if (position.y > SERVER_CONFIG.worldSize.height) position.y -= SERVER_CONFIG.worldSize.height;
...
serverEngine.createSystem(
  ...
  { all: [Position, Collider] },
  {
    ...
    after: () => {
      ...
      const players = serverEngine.getEntitiesWithTag('player').filter((p) => p.hasTag('alive'));
...
serverEngine.createSystem(
  ...
  { all: [Position, Health, PlayerData], tags: ['player', 'dead'] },
  {
    ...
    act: (entity: EntityDef, position: Position, health: Health) => {
      ...
      const timeSinceDeath = Date.now(); // Simplified - would track death time in real impl
    ...
    after: () => {
      ...
      if (worldState.length > 0) {
        broadcast({
          ...
          tick: Math.floor((currentTime - serverStartTime) / (1000 / SERVER_CONFIG.tickRate)),
          entities: worldState,
        });
      }
    },
...
function handleJoinRequest(client: any, message: any): void {
  ...
  console.log(`[Server] Player ${playerName} joined (${clientConnections.size}/${SERVER_CONFIG.maxPlayers})`);
}

/**
 * Handle player input
 */
function handlePlayerInput(client: any, message: any): void {
 ...
 */
function handleClientDisconnect(client: any): void {
 ...
 */
function sendWorldState(client: any): void {

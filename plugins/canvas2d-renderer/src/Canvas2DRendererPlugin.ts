...
import type { EnginePlugin, PluginContext, Entity } from '../../../core/src/index';
import type { Vector2, Bounds, Mesh, Color } from '../../../utils/src/index';

// Import utilities (these will be imported at runtime)
let Vector2Class: typeof Vector2;
let BoundsClass: typeof Bounds;

// Re-export utility types for consumers
export type { Vector2, Bounds, Mesh, Color };
...
export class Canvas2DAPI {
  ...
  public screenToWorld(
    ...
    const screenLeft = screenElement.unit === Unit.Percentage
      ? (screenElement.left / 100) * canvasWidth
      : screenElement.left;

    const screenTop = screenElement.unit === Unit.Percentage
...
export class Canvas2DRendererPlugin implements EnginePlugin {
  ...
  install(context: PluginContext): void {
    ...
    try {
      ...
      Vector2Class = utils.Vector2;
      BoundsClass = utils.Bounds;
    } catch (error) {
      console.warn('[Canvas2DRendererPlugin] Could not load utils, some features may not work');
    }
    ...
    context.createSystem(
      ...
      },
      {
        ...
        act: (entity: any, ...components: any[]) => {
          const [camera, transform, screenElement] = components as [Camera, Transform, ScreenElement];
          ...
          const screenWidth = screenElement.unit === Unit.Percentage
            ? (screenElement.width / 100) * canvasWidth
            : screenElement.width;

          const screenHeight = screenElement.unit === Unit.Percentage

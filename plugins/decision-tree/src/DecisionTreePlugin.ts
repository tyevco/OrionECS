...
export class DecisionTreePlugin implements EnginePlugin<{ decisions: DecisionTreeAPI }> {
    ...
    private getPredicateContext(): PredicateContext {
        if (!this.predicateContext) {
            ...
            this.predicateContext = {
                getSingleton: <T>(type: ComponentIdentifier<T>) => engine?.getSingleton?.(type),
                getEntity: (id: symbol) => engine?.getEntity?.(id),
                deltaTime: engine?.getDeltaTime?.() ?? 1 / 60,
            };
        }
    ...
    private createAPI(): DecisionTreeAPI {
        return {
            ...
            // Use property initializer instead of getter to avoid `this` context issues
            predicates: this.predicateRegistry,
        };
    }
}

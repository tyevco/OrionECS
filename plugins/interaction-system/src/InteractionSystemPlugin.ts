...
import type { Vector2, Bounds } from '../../../utils/src/index';

// Import utilities
let _Vector2Class: typeof Vector2;
...
export class InteractionSystemPlugin implements EnginePlugin {
  ...
  install(context: PluginContext): void {
    ...
    try {
      ...
      _Vector2Class = utils.Vector2;
      BoundsClass = utils.Bounds;
    } catch (_error) {
      console.warn('[InteractionSystemPlugin] Could not load utils');
    }
    ...
    if (!engine || !engine.input) {
      console.warn('[InteractionSystemPlugin] InputManagerPlugin not found. Interaction will not work.');
      return;
    }
  ...
  private handleDrag(context: PluginContext, delta: Vector2): void {
    ...
    if (draggable && draggable.isDragging) {
      if (draggable.onDrag) {
        ...
        // Try to get Transform from Canvas2DRenderer plugin
        const transform = (entity as any).transform || entity.getComponent('Transform' as any);
        if (transform) {
          transform.x += delta.x;
          transform.y += delta.y;
        }

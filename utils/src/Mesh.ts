...
export class Mesh {
   ...
   * @returns A new Mesh
   */
  public static rectangle(x: number, y: number, width: number, height: number, color?: Color): Mesh {
    const vertices = [
      new Vertex(x, y),
   ...
   * @returns A new Mesh
   */
  public static circle(centerX: number, centerY: number, radius: number, segments: number = 32, color?: Color): Mesh {
   ...
   * @returns A new Mesh
   */
  public static polygon(centerX: number, centerY: number, radius: number, sides: number, color?: Color): Mesh {
    const vertices: Vertex[] = [];
